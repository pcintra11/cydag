...
import { UnidadeNegocioModel as Model_Crud } from '../../../../appCydag/models';

const apiSelf = apisApp.unidadeNegocio;
  ...
  try {
    ...
    try {
      ...
      if (parm.cmd == CmdApi.list) {
        ...
        const documentConflict = await Model_Crud.findOne({ cod: data.cod } as Entity_Crud);
        if (documentConflict != null)
          throw new ErrorPlus('Unidade de Negócio já cadastrada.', { data: { fldName: Entity_Crud.F.cod } });
        ...
        if (documentDb == null)
          throw new ErrorPlus('Não foi encontrado a Unidade de Negócio.');
        const fldError = ValidateObjectFirstError({ ...data }, crudValidations);
        if (fldError != null)
          throw new ErrorPlus(fldError.msg, { data: { fldName: fldError.fldName } });
