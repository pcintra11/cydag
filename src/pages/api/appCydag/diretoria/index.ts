...
import { DiretoriaModel as Model_Crud } from '../../../../appCydag/models';

const apiSelf = apisApp.diretoria;
    ...
    try {
      ...
      if (parm.cmd == CmdApi.list) {
        ...
        if (documentConflict != null)
          throw new ErrorPlus('Diretoria já cadastrada.', { data: { fldName: Entity_Crud.F.cod } });
        ...
        if (documentDb == null)
          throw new ErrorPlus('Não foi encontrado a Diretoria.');
        const fldError = ValidateObjectFirstError({ ...data }, crudValidations);
        if (fldError != null)
          throw new ErrorPlus(fldError.msg, { data: { fldName: fldError.fldName } });
