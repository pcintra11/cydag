...
import { AgrupPremissasModel as Model_Crud } from '../../../../appCydag/models';

const apiSelf = apisApp.agrupPremissas;
    ...
    try {
      ...
      if (parm.cmd == CmdApi.list) {
        ...
        if (documentConflict != null)
          throw new ErrorPlus('Agrupamento de Premissas já cadastrado.', { data: { fldName: Entity_Crud.F.cod } });
        ...
        if (documentDb == null)
          throw new ErrorPlus('Não foi encontrado o Agrupamento de Premissas.');
        const fldError = ValidateObjectFirstError({ ...data }, crudValidations);
        if (fldError != null)
          throw new ErrorPlus(fldError.msg, { data: { fldName: fldError.fldName } });
