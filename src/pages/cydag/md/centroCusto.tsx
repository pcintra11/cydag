...
export default function PageCentroCustoCrud() {
  ...
  React.useEffect(() => {
    ...
    // CallApiCliASync(apisApp.userCrud.apiPath, globals.windowId, { cmd: CmdApi_UserCrud.list, getAll: true })
  ...
  try {
    if (mainStates.phase == Phase.list) {
      const colsGridConfig = [
        ...
        // new ColGridConfig('Responsável', ({ data }: { data: Entity }) => data.emailResponsavel),
      ];
    ...
    // const emailResponsavel = frmData.watch(Entity.F.emailResponsavel);
    // const usersAll = mainStates.users == null
    //   ? []
    //   : mainStates.users;
    // const usersActive = usersAll.filter((x) => x.ativo);

    // let optionsEmailResponsavel: User[];
    // let optionEmailResponsavelSelected: User;
    // optionsEmailResponsavel = usersActive;
    // optionEmailResponsavelSelected = null;
    // if (emailResponsavel != '') {
    //   if (mainStates.users == null)
    //     optionEmailResponsavelSelected = User.Generate({ email: emailResponsavel, nome: `${emailResponsavel} ...` });
    //   else {
    //     optionEmailResponsavelSelected = usersAll.find((x) => x.email == emailResponsavel);
    //     if (optionEmailResponsavelSelected == null ||
    //       !optionEmailResponsavelSelected.ativo) {
    //       if (optionEmailResponsavelSelected == null) {
    //         optionEmailResponsavelSelected = User.Generate({ email: emailResponsavel, nome: `${emailResponsavel} (NÂO ENCONTRADO)` } as User);
    //         optionsEmailResponsavel = [optionEmailResponsavelSelected, ...usersActive];
    //       }
    //       else if (!optionEmailResponsavelSelected.ativo) {
    //         optionEmailResponsavelSelected.nome += ' (INATIVO)';
    //         optionsEmailResponsavel = [optionEmailResponsavelSelected, ...usersActive];
    //       }
    //     }
    //   }
    // }
    // if (optionEmailResponsavelSelected == undefined) optionEmailResponsavelSelected = null;
      ...
      <Stack gap={1} height='100%' overflow='auto'>
        <form onSubmit={frmData.handleSubmit(phaseCrud.onSubmit)}>
          <Stack gap={1}>
            ...
            {/* <FrmCheckbox label='Ativo' frm={frmData} name={Entity.F.ativo} disabled={isDelete} /> */}
            {/* <FrmCheckbox label='SAP' frm={frmData} name={Entity.F.sap} disabled={isDelete} /> */}
            {/* <Autocomplete
              freeSolo={false} disabled={mainStates.users == null}
              value={optionEmailResponsavelSelected}
              renderInput={(params) => <TextField {...params} label='Responsável' />}
              options={optionsEmailResponsavel}
              onChange={(_event, newValue: User) => frmData.setValue(Entity.F.emailResponsavel, newValue?.email || '')}
