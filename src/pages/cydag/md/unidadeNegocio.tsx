  ...
  try {
    if (mainStates.phase == Phase.list) {
      const colsGridConfig = [
        ...
        new ColGridConfig(<Tx>Categ. Regional</Tx>, ({ data }: { data: Entity_Crud }) => <Tx>{CategRegionalMd.descr(data.categRegional)}</Tx>),
      ];
      ...
      <Stack spacing={1} height='100%' overflow='auto'>
        <form onSubmit={frmData.handleSubmit(phaseCrud.onSubmit)}>
          <Stack spacing={1}>
            ...
            <FormControl>
              <FormLabel id='demo-row-radio-buttons-group-label'>Categ. Regional</FormLabel>
              <RadioGroup row
                ...
                onChange={(ev) => {
                  ...
                  frmData.setValue(Entity_Crud.F.categRegional, value as CategRegional);
                }}
              // preparar como  em FrmInput
              >
                <FormControlLabel value={CategRegional.principais} disabled={isDelete} control={<Radio />} label={CategRegionalMd.descr(CategRegional.principais)} />
                <FormControlLabel value={CategRegional.outras} disabled={isDelete} control={<Radio />} label={CategRegionalMd.descr(CategRegional.outras)} />
                <FormControlLabel value={''} disabled={isDelete} control={<Radio />} label='nenhuma' />
              </RadioGroup>
            </FormControl>
            <BtnLine>
              {phaseCrud.button}
