...
export const MainCtrlModel = (() => {
  ...
  const schema = new mongoose.Schema<MainCtrl>({
...
export const SentMessageModel = (() => {
  ...
  const mongoose = MongooseSlot().mongoose;
  const schema = new mongoose.Schema<SentMessage>({
...
//interface SendMailLogMd extends mongoose.Document<ObjectId, any, SendMailLog> { }
export const SendMailLogModel = (() => {
  const modelName = modelNameSendMailLog;
  const mongoose = MongooseSlot().mongoose;
  const schema = new mongoose.Schema<SendMailLog>({
...
//interface ApiSyncLogMd extends mongoose.Document<ObjectId, any, ApiSyncLog> { }
export const ApiSyncLogModel = (() => {
  const modelName = modelNameApiSyncLogs;
  const mongoose = MongooseSlot().mongoose;
  const schema = new mongoose.Schema<ApiSyncLog>({
...
//export interface ApiAsyncLogMd extends mongoose.Document<ObjectId, any, ApiAsyncLog> { }
export const ApiAsyncLogModel = (() => {
  const modelName = modelNameApiAsyncLogs;
  const mongoose = MongooseSlot().mongoose;
  const schema = new mongoose.Schema<ApiAsyncLog>({
    appVersion: { type: String, required: false },
    apiHost: { type: String, required: false },
...
export const SiteEntryPointModel = (() => {
  ...
  const mongoose = MongooseSlot().mongoose;
  const schema = new mongoose.Schema<SiteEntryPoint>({
    date: { type: Date, required: true },
...
//interface ControlledAccessMd extends mongoose.Document<ObjectId, any, ControlledAccess> { }
export const ControlledAccessModel = (() => {
  const modelName = modelNameControlledAccess;
  const mongoose = MongooseSlot().mongoose;
  const schema = new mongoose.Schema<ControlledAccess>({
    browserId: { type: String, required: true },
...
export const SystemLogModel = (() => {
  ...
  const mongoose = MongooseSlot().mongoose;
  const schema = new mongoose.Schema<SystemLog>({
...
export const NotifyAdmCtrlModel = (() => {
  ...
  const mongoose = MongooseSlot().mongoose;
  const schema = new mongoose.Schema<NotifyAdmCtrl>({
    key: { type: String, required: true },
...
export const CacheModel = (() => {
  ...
  const mongoose = MongooseSlot().mongoose;
  const schema = new mongoose.Schema<CacheApp>({
...
export const DbTestModelX = ((collection: string, mongooseSlot?: string) => {
  ...
  const schema = new mongoose.Schema<DbTest>({
    key: { type: String, required: true },
...
export const JunkModel = (() => {
  ...
  const mongoose = MongooseSlot().mongoose;
  const schema = new mongoose.Schema<Junk>({
